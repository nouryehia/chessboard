...
class Checkoff(db.Model):
    ...
    def soft_delete(self) -> None:
        ...
        @author sravyabalasa
        '''
        # TODO: Delete all the CE for this checkoff as well?
        self.is_deleted = True
        self.save()
        return self

    def update_checkoff(self, description: str, name: str, points: int,
                        due: str) -> Checkoff:
        '''
        ...
        # TODO: Update the CE as well w/ updated score if points change
        self.save()
    ...
    def create_checkoff(description: str, name: str, course_id: int,
        ...
        c = Checkoff(description=description, name=name, course_id=course_id,
                     points=points, is_deleted=False,
                     status=Status.HIDDEN.value,
                     due=TimeUtil.convert_str_to_datetime(due))
        db.session.add(c)
        c.save()
    ...
    def find_all_checkoffs_in_course(course_id: int) -> List[Checkoff]:
        ...
        @author sravyabalasa
        """
        return Checkoff.query.filter_by(course_id=course_id, 
...
class CheckoffEvaluation(db.Model):
    ...
    def to_json(self):
        '''
        Function that takes a checkoff evaluation object and returns in a dict
        form. Used on the API layer.\n
        Params: None\n
    ...
    @staticmethod
    def find_latest_ce_for_checkoff_for_all_students(course_id: int,
                                                     checkoff_id: int) -> List[Tuple[User,CheckoffEvaluation]]:
        '''
        ...
        for student in students:
            ce = CheckoffEvaluation.\
                    find_latest_ce_for_checkoff_for_student(checkoff_id,
                                                            student.id)
    ...
    @staticmethod
    def find_latest_ce_for_all_checkoffs_for_student(course_id: int,
                                                     student_id: int) -> List[CheckoffEvaluation]:
        """
        ...
        for checkoff in checkoffs:
            ce = CheckoffEvaluation.\
                    find_latest_ce_for_checkoff_for_student(checkoff.id,
                                                            student_id)
    ...
    def find_latest_ce_for_checkoff_for_student(checkoff_id: int,
        ...
        return CheckoffEvaluation.query \
            .filter_by(checkoff_id=checkoff_id,
                       student_id=student_id).order_by(CheckoffEvaluation.
                                                       checkoff_time.desc()).first()
