...
from ...setup import login_manager, current_app
...
def course_role_required(crole=CRole.STUDENT.value):
    """
    Wrapper to be used for API calls that require the role of the user
    in a certain courese
    """
    def wrapper(fn):
        @wraps(fn)
        def decorated_view(*args, **kwargs):
